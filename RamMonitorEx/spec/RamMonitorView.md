...
- BackColor（初期値: Black）
  - ヘッダー行とコメント行の背景色も連動
- LabelForeColor（ラベル列の文字色、初期値: White）
- ValueForeColor（値列の文字色、初期値: White）
- UnitForeColor（単位列の文字色、初期値: White）
- CommentForeColor（コメント行の文字色、初期値: Gray）
- CommentBackColor（コメント行の背景色、初期値: Black）
  - ※現在未使用、将来的な拡張用
