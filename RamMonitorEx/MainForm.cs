...
namespace WindowsFormsApp1
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeMenu()
        {
            ...
            // ELFシンボル選択
            ToolStripMenuItem selectElfSymbolsItem = new ToolStripMenuItem("ELFシンボル選択...");
            selectElfSymbolsItem.Click += SelectElfSymbolsItem_Click;
            fileMenu.DropDownItems.Add(selectElfSymbolsItem);
        ...
        private void InitializeCommandBar()
        {
            ...
            var newGraphButton = new ToolStripButton("新規グラフ") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            newGraphButton.Click += (s, e) => AddNewGraphPaneWithName();

            var newRamButton = new ToolStripButton("新規RAMモニタ") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            newRamButton.Click += (s, e) => AddNewRamMonitorPaneWithName();

            var newMultiButton = new ToolStripButton("新規マルチレイアウト") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            newMultiButton.Click += (s, e) => AddNewMultiLayoutGridPaneWithName();

            var elfSelectButton = new ToolStripButton("ELFシンボル選択") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            elfSelectButton.Click += SelectElfSymbolsItem_Click;

            var openButton = new ToolStripButton("開く") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            openButton.Click += (s, e) => LoadWorkspace();
            ...
            _commandBar.Items.Add(newGraphButton);
            _commandBar.Items.Add(newRamButton);
            _commandBar.Items.Add(newMultiButton);
            _commandBar.Items.Add(elfSelectButton);
            _commandBar.Items.Add(new ToolStripSeparator());
            _commandBar.Items.Add(openButton);
            _commandBar.Items.Add(saveButton);

            this.Controls.Add(_commandBar);
        }
        ...
        private void CloseAllPanes()
        {
            ...
            {
                if (content is GraphPane || content is RamMonitorViewPane || content is MultiLayoutGridPane || content is ElfSymbolPane)
                {
        ...
        private void SelectElfSymbolsItem_Click(object? sender, EventArgs e)
        {
            if (TrySelectElfSymbols(out string elfFilePath, out List<ElfSymbolInfo> selectedSymbols))
            {
                ...
                AddNewElfSymbolPane(selectedSymbols, Path.GetFileNameWithoutExtension(elfFilePath));
            }
        }

        private void AddNewElfSymbolPane(List<ElfSymbolInfo> selectedSymbols, string sourceName)
        {
            string defaultName = PaneNameManager.Instance.RegisterNewName("ELFシンボルパネル");
            PaneNameManager.Instance.UnregisterName(defaultName);

            string paneName = defaultName;
            using (PaneNameDialog dialog = new PaneNameDialog(defaultName, "ELFシンボルパネル名を入力してください:"))
            {
                if (dialog.ShowDialog(this) != DialogResult.OK)
                {
                    return;
            ...
            ElfSymbolPane pane = new ElfSymbolPane(paneName, sourceName, selectedSymbols);
            pane.Show(_dockPanel, DockState.Document);
        }

        public bool TrySelectElfSymbols(out string elfFilePath, out List<ElfSymbolInfo> selectedSymbols)
        {
            using var dialog = new ElfSymbolSelectionForm();
            if (dialog.ShowDialog(this) == DialogResult.OK)
            {
                elfFilePath = dialog.SelectedElfFilePath;
                selectedSymbols = dialog.SelectedSymbols.ToList();
                return true;
            }

            elfFilePath = string.Empty;
            selectedSymbols = new List<ElfSymbolInfo>();
            return false;
        }
    }
}
